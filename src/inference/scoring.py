...
def score_month(month_data: pd.DataFrame, prev_buckets: dict,
    ...
    try:
        ...
        result = pd.DataFrame({
            ...
            "best_bucket": month_data["best_bucket"].values if "best_bucket" in month_data.columns else "—",
            "best_return": month_data["best_return"].values if "best_return" in month_data.columns else 0.0,
